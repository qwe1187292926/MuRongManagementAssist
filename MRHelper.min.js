...
// ==/UserScript==
const WORK_DAY="0",ON_WORK="01",TRAVEL_DICT={"1":"\u51FA\u5DEE","0":"\u6B63\u5E38"},WORK_DICT={"03":"\u4E8B\u5047","04":"\u75C5\u5047","05":"\u5A5A\u5047","02":"\u8C03\u4F11","00":"\u672A\u51FA\u52E4","12":"\u5C45\u5BB6\u529E\u516C","14":"\u80B2\u513F\u5047","11":"\u8FDC\u7A0B","06":"\u4E27\u5047","07":"\u966A\u4EA7\u5047","08":"\u4EA7\u5047","13":"\u75C5\u4F11","10":"\u4EA7\u68C0\u5047","09":"\u5E74\u5047","01":"\u221A"};let loginUser={oper_no:"",oper_pwd:"",oper_log_mod:"1",rad:""},MRCfg={resetFirstLoadRows:0,defaultLoginUser:{username:"",password:""},savedUsers:[],proId:"",welcomeWords:"\u667A\u80FD\u51FA\u52E4\u811A\u672C\u52A0\u8F7D\u6210\u529F\uFF01"};function initMRCfg(){let saved=GM_getValue("MR_CONFIG","");""==saved?GM_setValue("MR_CONFIG",MRCfg):mergeObject(saved,MRCfg),MRCfg=saved}function saveMRCfg(){GM_setValue("MR_CONFIG",MRCfg)}function setSavedUsers(username,password){console.log(username,password);let object={username:username,password:password};MRCfg.savedUsers.push(object),saveMRCfg()}function delSavedUser(name){MRCfg.savedUsers.splice(getIndexOfUser(name),1),saveMRCfg()}function getIndexOfUser(name){return MRCfg.savedUsers.findIndex(a=>a.username===name)}function clearSavedUsers(){MRCfg.savedUsers=[],saveMRCfg(),notify("\u6570\u636E\u5DF2\u6E05\u9664")}function setWorkStatus(att=""){let data=$("#murong-table");""==att&&data.bootstrapTable("checkAll");let rows=data.bootstrapTable("getSelections"),length=rows.length;for(let n=1;n<=length;n++){let row=rows[n-1];""==att?row.hld_flg==WORK_DAY&&(row.att_typ=ON_WORK):row.att_typ=att}refreshTable(),""==att?notify("\u5DF2\u81EA\u52A8\u52FE\u9009\u5DE5\u4F5C\u65E5\u4E3A\u51FA\u52E4\uFF01"):notify("\u9009\u4E2D\u884C\u5DF2\u52FE\u9009\u6307\u5B9A\u72B6\u6001")}function setProductInfo(proId){$HTTP("post","https://mis.murongtech.com/mrmis/attProjectQuery.do","search=&t="+Date.now()+"&limit=10&offset=0&totalRows=8&pro_nm="+proId,function(res){let data=(res=JSON.parse(res.responseText)).rec[0];if("1"==res.rec_num||confirm(`当前项目包含多个项目编号，是否选用最相近的结果：
项目编号：${data.pro_id}
项目名称：《${data.pro_nm}》
项目负责人：${data.att_man_nm}`)){console.log(res);let table=$("#murong-table");table.bootstrapTable("checkAll");let rows=table.bootstrapTable("getSelections"),length=rows.length;for(let n=1;n<=length;n++){let row=rows[n-1];for(let key in data)eval("row."+key+"= data['"+key+"']")}refreshTable(),MRCfg.proId=proId,saveMRCfg(),notify(`${proId} 项目编号已记录`)}else console.log("ads",res),notify("\u9879\u76EE\u67E5\u8BE2\u7ED3\u679C\u5305\u542B\u591A\u4E2A\u6216\u65E0\u7ED3\u679C\uFF01")},function(a){console.log(a),notify("\u8BF7\u6C42\u5931\u8D25\uFF0C\u8BE6\u89C1\u63A7\u5236\u53F0\uFF01")})}function $HTTP(method,url,data,onSuccess,onFailed){GM_xmlhttpRequest({method:method,url:url,data:data,headers:{Referer:getLocation(),"Content-Type":"application/x-www-form-urlencoded; charset=UTF-8",Cookie:document.cookie},onload:onSuccess,onerror:onFailed})}!function(){"use strict";if(initMRCfg(),isLoginPage()){let e=$("#oper_no"),f=$("#oper_pwd1");""!=MRCfg.defaultLoginUser.password&&""!=MRCfg.defaultLoginUser.username&&(notify("\u5DF2\u81EA\u52A8\u586B\u5145\u9ED8\u8BA4\u767B\u5F55\u8D26\u53F7"),f.val(MRCfg.defaultLoginUser.password),e.val(MRCfg.defaultLoginUser.username)),MRCfg.savedUsers.length>1&&""==MRCfg.defaultLoginUser.password&&notify("\u4F60\u53EF\u4EE5\u70B9\u51FB\u6807\u9898\u8BBE\u7F6E\u81EA\u52A8\u586B\u5145\u767B\u5F55\u7528\u6237\u6216\u8005\u8FDB\u884C\u591A\u8D26\u53F7\u7BA1\u7406\uFF01");let b=$($("button[type=submit]")[0]),d=$(`<a>清除已保存的用户</a>`);d.click(clearSavedUsers),$($("a[data-toggle]")[0]).parent().parent().append(d),$($("a[data-toggle]")[0]).parent().parent().append(logSavedUserData),b.removeAttr("type"),b.text("Save and login"),b.click(d=>{d.preventDefault();let b=$("#oper_no").val(),a=$("#oper_pwd1").val(),c=MRCfg.savedUsers.find(a=>a.username===b);void 0!=c?a!=c.password&&setSavedUsers(b,a):setSavedUsers(b,a),login(b,a)}),$("h3.form-title").click(function(){initTableSavedUsers()});return}welcome();let a=$("#toolbar"),c=`<li id="multiAccount"><a href="#"><i class="icon-user"></i>多账号管理</a></li>`;$("ul.nav li.user").before(c),c=`<li id="hoyoung_setting"><a href="#"><i class="icon-cogs"></i>脚本设置</a></li>`,$("ul.nav li.user").before(c),a.prepend(btnGenerator("hoyoung_set_status_data"," \u81EA\u9009\u6761\u4EF6\u586B\u5145","yellow-stripe","fa fa-check-square-o","\u5C06\u9009\u4E2D\u884C\u5E94\u7528\u8FD9\u4E2A\u51FA\u52E4\u72B6\u6001")),a.prepend(btnGenerator("hoyoung_set_product_data"," \u667A\u6167\u586B\u5145","green-stripe","fa fa-rocket","\u5C06\u8BE5\u9879\u76EE\u7F16\u53F7\u586B\u5165\u5230\u6240\u6709\u9879\u76EE\uFF0C\u5E76\u81EA\u52A8\u52FE\u9009\u51FA\u52E4\u72B6\u6001")),a.prepend('<input id="search_proid" placeholder="\u8BF7\u8F93\u5165\u9879\u76EE\u7F16\u53F7" style="margin-right: 5px;max-width: 8rem" class="m-wrap span5" type="text" value="'+MRCfg.proId+'"/>'),a.find("#hoyoung_set_product_data").click(function(){setProductInfo(a.find("#search_proid").val()),setWorkStatus()}),a.find("#hoyoung_set_status_data").click(function(){initConditionApply()}),$("#multiAccount").click(()=>{initTableSavedUsers()}),$("#hoyoung_setting").click(()=>{initSettingModal()}),0!=MRCfg.resetFirstLoadRows&&isChuQingPage()&&($("table#murong-table").bootstrapTable("getOptions").pageSize=MRCfg.resetFirstLoadRows,conditionQuery())}();let modalCurView="";function customMyModelView(html,title){if(0==$("#hoyoungModal").length){modalCurView=title;let modelTemplate=`
...
<div class="span3" style="
    ...
    * 日期的填写格式为YYmmdd，即年月日，需要满足八位长度（例如2022年1月1日对应的是20220101），毋须携带横杠
    ...
    `,"\u6309\u81EA\u9009\u89C4\u5219\u586B\u5145"),$("#hoyoung_custom_apply").click(function(){let a=-1,b=$("#murong-table"),c=b.bootstrapTable("getOptions").data;$.each(c,function(d,b){let c=parseInt(b.att_dt),e=$("#hoyoung_custom_start_date").val(),f=$("#hoyoung_custom_end_date").val();c>=e&&c<=f&&(-1===a&&(a=d),b.state=!0,b.att_typ=$("#hoyoung_custom_status_data").val(),b.travel_flg=$("#hoyoung_custom_travel_data").val())}),hideModal(),refreshTable(),console.log($("tbody tr")[a]),$("tbody tr")[a].scrollIntoView()})}function initSettingModal(){customMyModelView(`<div style="/* display:flex; *//* justify-content: flex-start; *//* align-content: center; *//* flex-wrap: nowrap; *//* flex-direction: row; */">
<div class="span3" style="
    width: 100%;
    display: flex;
    margin-left: 0;
...
<div class="span3" style="
    ...
    <p style="margin: 0">* 数据默认加载条数，默认为10条，当该值被设置为0时，不再劫持默认加载条数。</p>
      <p style="margin: 0">&nbsp;&nbsp;当脚本启动提示参数被设置为空时，启动完毕不再发出提醒。</p>
                ...
                `,"\u51FA\u52E4\u811A\u672C\u8BBE\u7F6E"),$("#hoyoung-save-setting").click(function(){$("input[name=hoyoung-setting]").each((i,obj)=>{let v=$(obj).val();"NaN"===parseFloat(v).toString()&&(v="'"+v+"'"),eval("MRCfg."+$(obj).attr("id")+"="+v),saveMRCfg(),hideModal()}),console.log(MRCfg)})}function hideModal(){$("#hoyoungModal").modal("hide")}function initTableSavedUsers(){customMyModelView(`
